...
def myMain():
    ...
    biggest = myMath.maximum(list_nums)
    smallest = myMath.minimum(list_nums)
    difference = myMath.subtraction(biggest, smallest)
    summation = myMath.add(biggest, smallest)
    total = myMath.sumTotal(list_nums)
    even = myMath.evenNum(list_nums)
    if smallest < 5:
         myMath.clear(list_nums)
    print(f'The difference is:%d The summation is:%d The summation of all input is:%d The number of even numbers is:%d The values in the list are: %s' % (difference, summation, total, even, list_nums))
